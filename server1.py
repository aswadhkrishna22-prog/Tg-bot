...
MAX_FILE_SIZE = 2 * 1024 * 1024 * 1024   # 6 GB

FILE_COOLDOWN = 20                        # 10 seconds
...
async def stats_command(event):
    ...
    try:
        ...
        # Adolf-StreamX bandwidth: bytes actually served to users.
        # This is persisted in the files table and is independent of
        # host/container-wide network interface counters.
        served_bandwidth_gb = 0.0
        try:
            with db_connect() as db:
                with db.cursor() as cursor:
                    cursor.execute(
                        "SELECT COALESCE(SUM(total_bytes_served), 0) AS total FROM files"
                    )
                    result = cursor.fetchone()
                    served_bytes = int(
                        result["total"] if result is not None else 0
                    )
                    served_bandwidth_gb = served_bytes / 1024**3
        except Exception as error:
            print("[STATS] Bandwidth query failed:", error)
            served_bandwidth_gb = 0.0
        ...
        message = (
            ...
            f"📡 <b>BANDWIDTH</b>\n"
            f"📤 DATA SERVED TO USERS: "
            f"<code>{served_bandwidth_gb:.2f} GB</code>\n"
            f"ℹ️ Measured from actual stream bytes; "
            f"persistent across restarts.\n\n"
...
async def receive_file(event):
    ...
    "💡 Want to know what this bot can do?\n"
    "👉 /help",
    buttons=buttons,
    parse_mode="html"
)
...
# ============================================================
# Existing hard-coded commands always win. Dynamic commands are only
# consulted after the known built-in command names have had a chance.
DYNAMIC_RESERVED_COMMANDS = {"start", "stats"}
DYNAMIC_MAX_PAGES = 20
DYNAMIC_PAGE_MAX_CHARS = 4096


def _dynamic_parse_mode(value):
    value = str(value or "HTML")
    return None if value == "plain" else value


def _dynamic_buttons(value):
    if not value:
        return None
    try:
        rows = value if isinstance(value, list) else json.loads(value)
    except Exception:
        return None
    ...
    for row in rows[:20]:
        ...
        for button in row[:4]:
            if not isinstance(button, dict):
                continue
            text = str(button.get("text", "")).strip()[:64]
            ...
            if not text or not (
                ...
                continue
            safe_row.append(Button.url(text, url))
        if safe_row:
            safe_rows.append(safe_row)
    return safe_rows or None


def _dynamic_decode_pages(response):
    """Decode a paged-command envelope; keep legacy plain responses working."""
    text = str(response or "")
    try:
        payload = json.loads(text)
    except Exception:
        return [text]
    if not isinstance(payload, dict) or payload.get("type") != "paged_command":
        return [text]
    pages = payload.get("pages")
    if not isinstance(pages, list) or not pages:
        return [text]
    clean = [str(page)[:DYNAMIC_PAGE_MAX_CHARS] for page in pages[:DYNAMIC_MAX_PAGES]]
    return clean or [text]


def _dynamic_page_buttons(command, page_index, total_pages, custom_buttons=None):
    rows = list(custom_buttons or [])
    nav = []
    if page_index > 0:
        nav.append(Button.inline("⬅️ Back", data=f"dynpage:{command}:{page_index - 1}".encode()))
    if page_index < total_pages - 1:
        nav.append(Button.inline("Next ➡️", data=f"dynpage:{command}:{page_index + 1}".encode()))
    if nav:
        rows.append(nav)
    return rows or None


async def _dynamic_send_page(event, command, page_index, edit=False):
    """Fetch a dynamic command and render one page plus navigation."""
    with db_connect() as db:
        with db.cursor() as cursor:
            cursor.execute(
                """SELECT response, parse_mode, buttons
                   FROM bot_commands
    ...
    if not row:
        if edit:
            await event.answer("Command is no longer available.", alert=True)
        return False

    pages = _dynamic_decode_pages(row.get("response"))
    page_index = max(0, min(int(page_index), len(pages) - 1))
    parse_mode = _dynamic_parse_mode(row.get("parse_mode"))
    custom_buttons = _dynamic_buttons(row.get("buttons"))
    buttons = _dynamic_page_buttons(
        command, page_index, len(pages), custom_buttons
    )

    kwargs = {"buttons": buttons} if buttons else {}
    if parse_mode:
...
@bot.on(events.NewMessage(pattern=r"^/([a-zA-Z0-9_]{1,32})(?:@[^\s]+)?(?:\s.*)?$"))
async def dynamic_command_fallback(event):
    if not BOT_MODE or event.file:
        return
    match = event.pattern_match
    command = (match.group(1) or "").lower()
    if command in DYNAMIC_RESERVED_COMMANDS:
        return
    try:
        await _dynamic_send_page(event, command, 0, edit=False)
    except Exception as error:
        print("[COMMANDS] Dynamic lookup failed:", error)
        return


@bot.on(events.CallbackQuery(pattern=rb"^dynpage:"))
async def dynamic_page_callback(event):
    """Handle Back/Next navigation for dynamic multi-page commands."""
    try:
        ...
        if command in DYNAMIC_RESERVED_COMMANDS:
            await event.answer("This command is reserved.", alert=True)
            return
        page_index = int(parts[2])
        if page_index < 0 or page_index >= DYNAMIC_MAX_PAGES:
            await event.answer("Invalid page.", alert=True)
            return

        ok = await _dynamic_send_page(event, command, page_index, edit=True)
        if ok:
            await event.answer()
    except (ValueError, IndexError):
        await event.answer("Invalid page.", alert=True)
    except Exception as error:
        print("[COMMANDS] Dynamic page failed:", error)
        await event.answer("Unable to open this page.", alert=True)
